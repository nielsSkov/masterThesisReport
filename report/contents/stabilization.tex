...
\end{figure}
%
Finally the swing-up controller and the sliding mode controller are simulated in concert, where the sliding mode controller is activated at a catch angle of \SI{0.1}{rad \cdot s^{-1}}. The result is seen in \autopageref{fig:theta_swingAndCatch} and \autoref{fig:x_swingAndCatch}, where the swing-up controller brings the angle below the catch angle in seven swings, after which the system is stabilized in zero by the sliding mode controller.
%
%AVAILABLE PLOTS FROM THIS SIMULATION
%x_swingAndCatch
%xDot_swingAndCatch
